...
struct MeteoritiEbashat: View {
    ...
    var body: some View {
        ZStack {
                ...
                LottieView(animation: .named("explosionMeteorits"))
                .playbackMode(playbackMode)
                    ...
                    .scaleEffect(animationFinished ? 0 : 1)
                    .opacity(!tapped ? 0 : 1)
                    .disabled(tapped)
            
        }
        .position(go ? position : CGPoint(x: CGFloat.random(in: 0...UIScreen.main.bounds.width), y: -900))
            .onAppear {
                let timeDuration = TimeInterval.random(in: 3...10)
                withAnimation(Animation.linear(duration: timeDuration)) {
                    go = true
                    print (position)
                }
                
                DispatchQueue.main.asyncAfter(deadline: .now() + timeDuration) {
                    if !tapped {
                        if resultWin == nil {
                            action()
                        }
    ...
    func body(content: Content) -> some View {
        ZStack {
            if !isLoading {
                if PSofe != nil {
                    VStack(spacing: 0) {
                        WKWebViewRepresentable(url: PSofe!, webView: webView)
                        HStack {
                            Button(action: {
                                webView.goBack()
                            }, label: {
                            ...
                            Button(action: {
                                ...
                                webView.load(URLRequest(url: PSofe!))
                            }, label: {
...
//        .yesMo(orientation: .all)
    ...
    func gudPea() {
        guard let url = URL(string: "https://optimizeprivacyonline.online/en/plo") else {
            DispatchQueue.main.async {
...
extension View {
    func kslea() -> some View {
        self.modifier(Kseofap())
    }
